...
class MechanismParameters3Dof {
   ...
   */
  public readonly local_position_plat_ball_joint_right: THREE.Vector3;
  /**
   * the position of the platform's left anchor/balljoint relative to the platform's pivot center
   * This is one of two balljoints used to control pitch/roll.
   * x,y,z = lat, vert, long distance
   */
  public readonly local_position_plat_ball_joint_left: THREE.Vector3;
  /**
   ...
   * x,y,z = lat, vert, long distance
   */
  public readonly local_position_servo_yaw: THREE.Vector3;
  /**
   ...
   */
  constructor(
    local_position_plat_ball_joint_right: THREE.Vector3 = new THREE.Vector3(
      -35.6235,
      0,
      -29.9974
    ),
    local_position_plat_ball_joint_left: THREE.Vector3 = new THREE.Vector3(
      35.6235,
      0,
      -29.9974
    ),
    local_position_plat_stand_ball_joint: THREE.Vector3 = new THREE.Vector3(
      0,
      -47.4726,
      -8.636
    ),
    local_position_servo_pitch_roll_right: THREE.Vector3 = new THREE.Vector3(
      -10.7442,
      -28.2194,
      -29.9974
    ),
    local_position_servo_pitch_roll_left: THREE.Vector3 = new THREE.Vector3(
      10.7442,
      -28.2194,
      -29.9974
    ),
    ...
    local_position_servo_yaw: THREE.Vector3 = new THREE.Vector3(
      -25.2222,
      -47.4726,
      5.461
    ),
 ...
 */
class Mechanism3Dof extends THREE.Group {
  ...
  getYawConnectingRodLength(): number {
    return this.servo_Yaw.horn.ballJoint
      ...
      .distanceTo(
        this.platformStand.ballJoint.getWorldPosition(new THREE.Vector3())
      );
  }

  getYawConnectingRodMidPoint_WorldPosition(): THREE.Vector3 {
    return lerpVector(
      this.servo_Yaw.horn.ballJoint.getWorldPosition(new THREE.Vector3()),
      this.platformStand.ballJoint.getWorldPosition(new THREE.Vector3()),
      0.5
    );
  }
  ...
  getConnectingRodLength_Left(): number {
    return this.servo_PitchRoll_left.horn.ballJoint
      .getWorldPosition(new THREE.Vector3())
      .distanceTo(
        this.platform.ballJoint_left.getWorldPosition(new THREE.Vector3())
      );
  }
  ...
  getConnectingRodLength_Right(): number {
    return this.servo_PitchRoll_right.horn.ballJoint
      .getWorldPosition(new THREE.Vector3())
      .distanceTo(
        this.platform.ballJoint_right.getWorldPosition(new THREE.Vector3())
      );
  }

  getConnectingRodMidPoint_Left_WorldPosition(): THREE.Vector3 {
    return lerpVector(
  ...
  getConnectingRodMidPoint_Right_WorldPosition(): THREE.Vector3 {
    return lerpVector(
  ...
  getBVec_Left(): THREE.Vector3 {
    // initialize with the world position of the center of servo arm rotation
    // then convert the world position to local "base" space
    return this.base.worldToLocal(
      this.servo_PitchRoll_left.horn.getWorldPosition(new THREE.Vector3())
    );
  }

  // base frame vector from origin of base to center of servo arm rotation
  getBVec_Right(): THREE.Vector3 {
    // initialize with the world position of the center of servo arm rotation
    // then convert the world position to local "base" space
    return this.base.worldToLocal(
      this.servo_PitchRoll_right.horn.getWorldPosition(new THREE.Vector3())
    );
  }
  ...
  getYawServoAngle(): number {
    ...
    const p_local_desiredEndEffector = new THREE.Vector3();
  ...
  setFinalOrientation(yaw: number, pitch: number, roll: number): void {
    ...
    this.platform.setRotationFromEuler(new THREE.Euler(pitch, 0, roll, 'XYZ'));
  ...
  trackTarget(targetWorldPosition: THREE.Vector3): void {
    const sourcePoint = this.platform.getWorldPosition(new THREE.Vector3());
...
class PlatformStand extends THREE.Group {
  ...
  constructor(local_position_ball_joint: THREE.Vector3) {
    super();
    // Ball Joint
...
class Platform extends THREE.Group {
  ...
  constructor(
    local_position_ball_joint_right: THREE.Vector3,
    local_position_ball_joint_left: THREE.Vector3
  ) {
    super();
